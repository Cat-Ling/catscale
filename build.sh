...
merge_info_plist() {
  ...
  if [ -d "$target_app" ] && [ -f "$src_info" ]; then
    echo "📋 Resolving bundle variables and enforcing complete Info.plist metadata from canonical $src_info..."
...
try:
    # 1. Load canonical Info.plist (Single Source of Truth)
    with open(src_path, 'rb') as f:
        canonical = plistlib.load(f)

    # 2. Load destination plist (if generated by toolchain)
    dest = {}
    ...
    # 3. Dynamic replacement dictionary derived strictly from canonical source
    replacements = {
        '$(PRODUCT_NAME)': canonical.get('CFBundleName', 'Catscale'),
        '$(PRODUCT_BUNDLE_IDENTIFIER)': canonical.get('CFBundleIdentifier', 'com.catscale.app'),
        '$(EXECUTABLE_NAME)': canonical.get('CFBundleExecutable', 'Catscale'),
        '$(TARGET_NAME)': canonical.get('CFBundleName', 'Catscale'),
        '$(MARKETING_VERSION)': str(canonical.get('CFBundleShortVersionString', '0.0.1')),
        '$(CURRENT_PROJECT_VERSION)': str(canonical.get('CFBundleVersion', '1')),
        '$(DEVELOPMENT_LANGUAGE)': canonical.get('CFBundleDevelopmentRegion', 'en'),
    }

    # 4. Resolve any $(...) template strings in existing keys
    ...
    # 5. Overlay canonical settings to guarantee 100% compliance
    dest.update(canonical)
    dest['UIDeviceFamily'] = [1, 2]
    if 'UILaunchScreen' not in dest or not isinstance(dest['UILaunchScreen'], dict):
        dest['UILaunchScreen'] = {}

    with open(dest_path, 'wb') as f:
        plistlib.dump(dest, f)
    print(f"✅ Successfully synchronized Info.plist at {dest_path} (Version: {dest.get('CFBundleShortVersionString')})")
